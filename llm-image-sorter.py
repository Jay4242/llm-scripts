...
def build_messages(
    subdirectories: list[str], image_name: str, base64_image: str
) -> list[dict]:
    ...
    messages = [
        {"role": "system", "content": system_prompt},
        {
            ...
            "content": [
                {
                    "type": "text",
                    "text": f"I'm about to send you an image named: {image_name}",
                }
            ],
        },
        {
        ...
        },
        {
            ...
            "content": [
                {
                    "type": "text",
                    "text": f"Available subdirectories to sort it into:\n{subdir_list}",
                }
            ],
        },
        {
            ...
            "content": [
                {
                    "type": "text",
                    "text": 'Examine the image and tell me which subdirectory it belongs in. Respond with a JSON array containing exactly one element - the subdirectory name. Format: ["subdirectory_name"]',
                }
            ],
        },
    ]
...
def query_llm(
    ...
    client = OpenAI(base_url=base_url, api_key=api_key, timeout=httpx.Timeout(timeout))
...
def strip_think_tags(response: str) -> str:
    ...
    # Remove think tags and their content (non-greedy, case-insensitive)
    cleaned = re.sub(
...
def main():
    ...
    parser.add_argument(
        "source_dir", help="Source directory containing images and subdirectories"
    )
    parser.add_argument(
        "--base-url",
        "-b",
    ...
    parser.add_argument(
        "--model", "-m", default="qwen3.5", help="Model name to use (default: qwen3.5)"
    )
    parser.add_argument(
        "--api-key", "-k", default="", help="API key for the backend (optional)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    for i, image_name in enumerate(images, 1):
        ...
        try:
            raw_response = query_llm(messages, args.base_url, args.model, args.api_key)
        except Exception as e:
        ...
        category = parse_llm_response(cleaned_response)
        ...
        if args.dry_run:
            print(
                f"  (dry run) Would move to: {args.source_dir}/{category}/{image_name}"
            )
        else:
